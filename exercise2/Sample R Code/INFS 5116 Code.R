...
ggplot(df,aes(x=df$PRICE, y=df$PROFIT, group=pt,shape=pt,size=radius,colour=df$Price_Tier)) +
              scale_shape_manual(values=c(15,16,17,18)) + scale_colour_manual(values=cbbPalette) + theme(panel.background = element_rect(fill ="white"), panel.grid.major = element_line(colour = "#BFC1C9")) + geom_point( alpha=0.8) +  guides(colour = FALSE, shape = guide_legend(order = 1)) + labs(shape = "Price Tier",size="Scale") +
...
ggplot(df, aes(df$PRICE, y=df$PROFIT, colour=df$Price_Tier, size=radius)) + 
    geom_point( alpha=0.8) + scale_colour_brewer(palette="Spectral", type="qual",name="Price Tier") +
...
beeswarm(PRICE ~ Price_Tier, data = df1,
        method = 'hex', corral = "gutter",
         pch = 16, pwcol = col,
         xlab = '', ylab = 'Price',cex=1.5,cex.lab=1.8)
...
# Now for the plot
ggplot(ch, aes(monthweek, weekdayf, fill = Profit)) + 
    geom_tile(colour = "white") + facet_grid(year~monthf) + scale_fill_gradientn(colours = rev(pal(10))) +
    theme(panel.background = element_rect(fill ="white"), panel.grid.major = element_line(colour = "#BFC1C9"),axis.title=element_text(size=14),axis.text=element_text(size=10))+ 
